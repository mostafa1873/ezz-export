...
const MovingTicker = memo(() => {
    ...
    return (
        <div className="relative w-full bg-[#0a2e1a] py-6 mt-5 lg:mt-0 mb-7 overflow-x-hidden ltr:rotate-[-2deg] rtl:rotate-[2deg] scale-[1.02] md:scale-[1.05] z-50" dir="ltr">
            <motion.div
                animate={{ x: ["0%", "-50%"] }}
                transition={{
                    repeat: Infinity,
                ...
                className="flex whitespace-nowrap gap-12 md:gap-24 min-w-max"
            >
                {[...Array(4)].map((_, i) => (
                    <div key={i} className="flex items-center gap-12 md:gap-24" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-[0.04] select-none pointer-events-none">
                <h2 className="text-[30vw] font-black tracking-tighter text-green-900 uppercase">EZZ</h2>
            </div>
                    ...
                    <div className="w-full lg:w-1/2 text-center lg:ltr:text-start lg:rtl:text-right z-30">
                        ...
                        >
                            ...
                            <h1 className="text-[11vw] sm:text-[8vw] lg:text-[5.5rem] font-black text-slate-900 leading-[0.95] rtl:leading-[1.1] tracking-tighter rtl:tracking-normal mb-6">
                                {t('titlePrefix')} <br />
                                <span className="text-[#2d5a27] relative">{t('titleHighlight')}</span> <br />
